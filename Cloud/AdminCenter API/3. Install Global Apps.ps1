...
$EnvironmentsToUpdate | ForEach-Object {
    ...
    foreach ($App in $AppIdsToInstall) {
        ...
        Write-Host "Install app $i of $NoOfApps : $App"

        Invoke-WebRequest `
            -Method Post `
            -Uri    "https://api.businesscentral.dynamics.com/admin/$adminVersion/applications/$applicationFamily/environments/$environment/apps/$App/install" `
            -Body   (@{
